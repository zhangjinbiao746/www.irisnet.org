        ...
        <div class="warp">
          <div class="center_content">
            <div class="center-content-img">
              <img src="../assets/GOG-landscape-transparent.png" />
            </div>
            <div class="join-title">
              <h5>{{ title }}</h5>
            </div>
            <div class="introduction-container-wrap">
            ...
            <div class="rule-info-container-wrap">
              <ul class="rule-info-container-list">
                <li>{{ signupDate }}</li>
                <li class="rule-info">{{ join }}</li>
                <li class="riot-info">
              ...
              <div class="button-container">
                <div class="sign-btn">
                  <a
                    :href="
                      $store.state.lang === 'EN'
                        ? 'http://cn.mikecrm.com/loO06Op'
                        : 'http://cn.mikecrm.com/Wdawxt7'
                    "
                    target="_blank"
                    >{{ btn1 }}</a
                  >
                ...
                <div class="read-btn">
                  <a
                    :href="
                      $store.state.lang === 'EN'
                        ? 'https://medium.com/@kidinamoto/irisnet-pos-security-hackathon-707065865926'
                        : 'https://github.com/irisnet/testnets/issues/49'
                    "
                    target="_blank"
                    >{{ btn2 }}</a
                  >
...
export default {
  ...
  data() {
    return {
      ...
      award:
        "奖励机制： 参赛的各组选手瓜分的攻防大赛瓜分212,500IRIS枚通证的奖金池。 比赛中按规则排名划分项目一二三等奖，一等奖3组，奖励25,000枚IRIS通证/组；二等奖5组，奖金12,500枚IRIS通证/组；三等奖10组，奖金6,250枚IRIS通证/组,阳光普照奖100组奖金125枚IRIS通证/组",
      partake: "参与方式： 加入官方QQ群： 862553695",
      // join:'',
      btn1: "点击报名",
      btn2: "阅读详情",
      src:
        message[this.$store.state.lang == "CN" ? "cn" : "en"].newHackathon.src,
      introduction:
        "活动介绍：为了鼓励更多的技术人员加入IRISnet社区并且参与到测试网中，IRISnet和Dorahacks正在寻找最优秀的黑客来参加到这场攻防游戏中。让我们一起来寻找使区块链网络更加安全可靠的新解决方案吧！",
      signupDate: "报名进行中",
  ...
  methods: {
    format(param) {
      return message[this.$store.state.lang == "CN" ? "cn" : "en"].newHackathon[
        param
      ];
    },
    getInfo() {
      //title,introduction,signupDate，join，riotRoom，bountyPool
...
.rule-info-container-wrap {
  width: 100%;
  padding-top: 0.29rem;
  .rule-info-container-list {
    margin: 0 auto;
    color: #d4d5de;
    font-size: 0.14rem;
    width: 4.87rem;
